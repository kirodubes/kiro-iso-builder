...
### What Changed
On a machine that does not have the `kiro-iso` clone yet, ticking **plasma** on Configure did
...
`#e0567a`) in `style.css`; the dialog reuses the existing `.dim-label` class. Identical
change applied to the nemesis (beta) builder to keep the two in lockstep.
...
- The build log was leaking the literal text `]3008;start=…;comm=sudo;targetuser=root…` and a
  matching `]3008;end=…\` line around each `sudo` invocation. These are OSC command-tracking
...
- `functions.py`: `_ANSI_RE` OSC branch changed from `\x1b\][^\x07]*\x07` to
  `\x1b\][^\x07\x1b]*(?:\x07|\x1b\\)` — matches an OSC string up to the first BEL **or** ST. The
...
- **Reverted the "Fresh disk" QEMU toggle** added earlier today. The QEMU test disk is
  back to being wiped on every "Test in QEMU" click (`disk.unlink()`), so every run
  starts from a clean target — matching VirtualBox. Reason: with QEMU's `-bios` firmware
  mode the UEFI NVRAM boot entry is not persisted, so a reused disk still loops back into
  the ISO rather than booting the install; the persistence added complexity without
  delivering a bootable installed system.
...
- **`done_gui.py`** — added a **Burn to USB** action to the Done screen. When
  ...
  install-or-launch pair. Synced from the nemesis source after live testing.
...
## 2026-06-08 — Never move/adopt the HQ source repo

### What Changed
- **Removed the repo-move behaviour** (`preflight_gui.py`): picking a folder no
  longer offers to `shutil.move` an existing clone — it just points the app at
  the chosen target and clones a fresh copy there if needed. The
  `_confirm_move`/`_on_move_confirmed`/`_on_moved` helpers + the `shutil` import
  are gone.
- **Dropped the HQ-sibling auto-discovery** (`functions.py find_build_scripts`):
  the builder no longer adopts `APP_DIR.parent/<repo>`. On the dev box that is a
  Kiro-HQ source folder (`KIRO-ISO-CALAMARES/kiro-iso`); it now defaults to
  `~/<repo>` instead.

### Why
- Run on **hq**, the builder discovered the sibling `KIRO-ISO-CALAMARES/kiro-iso`
  and then *moved* it to the chosen/default spot — twice scattering the source
  repo into `~/Music`, `~/Desktop`, `~/kiro-iso`. The builder must **never move,
  adopt, or build inside** a Kiro-HQ source folder. It now stays entirely on
  `~/kiro-iso` (or an explicitly chosen non-HQ location) and only ever clones.

### Files Modified
- `preflight_gui.py` — drop move helpers + `shutil` import
- `functions.py` — drop sibling-clone discovery candidate
